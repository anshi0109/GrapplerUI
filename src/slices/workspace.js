...
async function handleDelete(workspaceId, companyId) {
  ...
  function demo(id){
    console.log(id);
    //navigate(`/addproject?id=${workspaces.id}`)
  }
      ...
      <Modal.Body>
        <Form>
          <Form.Group controlId="workspaceName">
            ...
            <Form.Control type="text" placeholder="Enter workspace name" id="name" required/>
          </Form.Group>
        </Form>
      </Modal.Body>
      <Modal.Footer>
        ...
        <Button variant="primary" type="submit" onClick={() => {addWorkspace(document.getElementById('name').value)}}>
          Create Workspace
        </Button>
      </Modal.Footer>
    </Modal>
  ...
  return (
    <div>
      <div className="container">
        ...
        <Button onClick={() =>demo(workspaces.id)}>Projects</Button>
        <ToastContainer />
